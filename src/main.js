...
io.on('connect', function(socket){
  ...
  socket.on('register', function(name, regID){
    ...
    MonopolyGame.cleanBoard();//test
    MonopolyGame.runTurn(MonopolyGame.PlayerArray[0]);//test
  });
  ...
  MonopolyGame.Emitter.on('promptRoll', function(ActivePlayer){
    ...
    console.log('rollprompt to ' + ActivePlayer.socketID);
    socket.emit('promptRoll', MonopolyGame.boardState());
  });
